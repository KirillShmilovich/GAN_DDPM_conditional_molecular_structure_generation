...
class GaussianDiffusion(nn.Module):
    def __init__(self, denoise_fn, timesteps=1000, loss_type='l1', betas = None, beta_schedule='linear', unmask_number = 0):
        super().__init__()
    ...
    def p_sample_loop(self, shape, samples = None, save_diffusion_progress=False):
        ...
        if save_diffusion_progress:
            ...
            np.save('./results/train_pep_AA-heavy/32-1-2-4-8-b128/AA-heavy_Unet1D_2000000s/diff_progess.npy', 
                    np.array(state_list))
...
class Dataset_traj(torch.utils.data.Dataset):
  ...
  def __init__(self, folder, system, n_conds=None):  #MJ add n_conds if using when passing in conds
        super().__init__()
        self.folder = folder
  ...
  def __getitem__(self, index):
        ...
        if self.n_conds is not None:
            x_c = 2*x/(self.max_data[:self.n_conds] - self.min_data[:self.n_conds])
            x_c = x_c - 2*self.min_data[:self.n_conds]/(
                self.max_data[:self.n_conds]-self.min_data[:self.n_conds]) - 1
            x = np.concatenate(x_c, x[self.n_conds:])
            print(x_c.shape, x.shape)
...
class Trainer(object):
    ...
    def load(self, milestone, device=torch.device("cuda")):
        model_data = torch.load(str(self.RESULTS_FOLDER / f'model-{milestone}.pt'),  # MJ added map for cpu
                               map_location=device)
