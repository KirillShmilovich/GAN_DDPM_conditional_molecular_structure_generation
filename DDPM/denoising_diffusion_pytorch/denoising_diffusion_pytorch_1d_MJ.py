...
## Added through EMA() below from the Pratush masking adaptation
...
## try witihout conditions for now?
## (no unmask number or inprint mask0) 
...
class Dataset_traj(torch.utils.data.Dataset):
  ...
  def __init__(self, folder, system, n_conds=None):  #MJ add n_conds when passing in conds
        super().__init__()
        self.folder = folder
  ...
  def __getitem__(self, index):
        ...
        # normalize the conditions only
        if self.n_conds is not None:
            min_c, max_c = self.min_data[:self.n_conds], self.max_data[:self.n_conds]
            x_c = 2*x/(max_c - min_c)
            x_c = x_c - 2*min_c/(max_c-min_c) - 1
           
            # just need to append values to match input dims
            x_z = np.zeros((1, len(self.min_data)-self.n_conds))
            x = np.concatenate([x_c, x_z], axis=-1)
...
# trainer class -- imported from Pratush
...
class Trainer(object):
    ...
    def load(self, milestone, device=torch.device("cuda")):
        model_data = torch.load(str(self.RESULTS_FOLDER / f'model-{milestone}.pt'),  # MJ added for cpu
                               map_location=device)
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            # MJ is this just to print/save outputs? op_num here doesn't really matter?
